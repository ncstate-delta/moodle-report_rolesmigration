...
if ($roles_in_file = roles_migration_get_incoming_roles()) {
    foreach ($roles_in_file as $role) {
        if (!isset($_POST['actions'][$role->shortname])) {
            echo '<p>', get_string('role_ignored', 'report_rolesmigration', $role), '</p>';
            continue;
        }
        
        switch ($_POST['actions'][$role->shortname]) {
            case 'skip':
                echo '<p>', get_string('role_ignored', 'report_rolesmigration', $role), '</p>';
                break;
            case 'create':
            ...
            default:
                $a = new stdclass();
                $a->action = $_POST['actions'][$role->shortname];
